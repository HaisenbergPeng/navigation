...
public:
    ...
    float startTemporaryMappingErrorThre; // if no serious map out-of-date, it suffice 99% of the time
    float startTemporaryMappingInlierRatioThre; // 
    int slidingWindowSize;

    ParamServer()
    {
        nh.param<bool>("kloam/debugMode", debugMode, false);

        nh.param<float>("kloam/starttemporaryMappingDistThre", starttemporaryMappingDistThre, 20.0);
        nh.param<float>("kloam/startTemporaryMappingErrorThre", startTemporaryMappingErrorThre, 0.2);
        ...
        nh.param<std::string>("kloam/gpsTopic", gpsTopic, "odometry/gps");
        ...
        nh.param<std::string>("kloam/baseFootprintFrame", baseFootprintFrame, "base_footprint");
        ...
        nh.param<bool>("kloam/mapLoaded", mapLoaded, false);
        nh.param<bool>("kloam/relocSuccess", relocSuccess, false);
        ...
        nh.param<vector<double>>("kloam/transBodyLidar", transBodyLidar, vector<double>());
        nh.param<vector<double>>("kloam/initialGuess", initialGuess, vector<double>());
