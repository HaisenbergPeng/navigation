...
bufferLen=21 # including the start bytes
...
class myOdometry(object):
    ...
    #    self.buffer = self.buffer[12:]
        ...
        with self.lock:
            ...
            self.x, self.y, self.theta,_,_,self.v_cur,self.w_cur,self.controlstate =  struct.unpack('>2i5h1B', str[0:bufferLen-2])
    ...
    def write (self):
        with self.lock:
            ...
            self.ser.write(data);
            self.ser.read(3); # read 3 characters
